...
// Thời gian kết thúc Flash Sale (có thể đổi thành +1 days, +2 hours, v.v.)
$flash_sale_end = strtotime('+1 days');

// === SỬA CHÍNH TẠI ĐÂY - PHÙ HỢP HOÀN TOÀN VỚI DATABASE CỦA BẠN ===
$new_products      = getNewProducts($db, 8);        // Sản phẩm mới
$featured_products = getNewProducts($db, 12);       // Khuyến mãi Hot (tạm dùng sản phẩm mới vì chưa có compare_price)

$categories = getCategories($db);
$brands     = getBrands($db);     // trả [] → không lỗi
$posts      = getLatestPosts($db); // trả [] → không lỗi
?>
<!DOCTYPE html>
<html lang="vi">
<head>
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* BANNER, DANH MỤC, PROMO, PRODUCT CARD... (giữ nguyên toàn bộ CSS cũ của bạn) */
  ...
  @media (max-width: 992px) { .col-3, .col-4 { flex: 0 0 50%; max-width: 50%; } }
  @media (max-width: 768px) { body { padding-top: 180px; } .col-3, .col-4, .col-6 { flex: 0 0 100%; max-width: 100%; } .banner { flex-direction: column; height: auto; } }
</style>
...
<body>
    ...
    <div class="grid">
        ...
        <div class="banner__left">
          <div class="banner__carousel" id="bannerCarousel">
            <div class="carousel__slide active">
              <img src="assets/images/banners/banner1.webp" alt="Banner 1" class="carousel__img">
            </div>
            <div class="carousel__slide">
              <img src="assets/images/banners/banner2.webp" alt="Banner 2" class="carousel__img">
            </div>
            <div class="carousel__slide">
              <img src="assets/images/banners/banner3.webp" alt="Banner 3" class="carousel__img">
            </div>
            <button class="carousel__btn carousel__prev" onclick="changeSlide(-1)">Left</button>
            <button class="carousel__btn carousel__next" onclick="changeSlide(1)">Right</button>
            ...
            <div class="grid__col col-3">
              <div class="product-card">
                <div class="product-image">
                  <img src="../admin/assets/images/uploads/<?= htmlspecialchars($image) ?>" 
                ...
                <div class="product-info">
                  ...
                  <div class="product-actions">
                    <button class="btn btn-outline-primary action-btn" data-id="<?= $product['id'] ?>" data-action="buy">Mua ngay</button>
                    <button class="btn btn-primary action-btn" data-id="<?= $product['id'] ?>" data-action="cart">Thêm vào giỏ</button>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section class="section">
        <div class="d-flex justify-content-between align-items-center mb-4">
          <h2 class="section__title text-danger mb-0">Flash Sale - Giảm Sốc</h2>
          <div class="flash-sale-timer">
            Kết thúc sau: <span id="countdown-timer" data-end="<?= $flash_sale_end ?>"></span>
          </div>
        </div>
  ...
  <script>
        ...
        if (isLoggedIn === 'true') {
          $.post('ajax/add_to_cart.php', { product_id: productId, quantity: 1 }, function(res) {
            if (res.success) {
              ...
              showNotification(res.message || 'Lỗi!', 'error');
            }
          }, 'json');
        } else {
    ...
    function showNotification(message, type = 'info') {
      ...
      const html = `<div class="alert ${alertClass} alert-dismissible fade show position-fixed" style="top:100px;right:20px;z-index:9999;min-width:300px;">
